# `buildings/` — the complete pipeline
...
│   └── _vendored/
│       ├── lsh.py             byte-for-byte copy of lane_a/core/lsh.py
│       └── bloom.py           byte-for-byte copy of lane_a/core/bloom.py
│
...
**Drop both into `buildings/corpus/`:**
...
`respond_node.py`) then finds them there with no flags. Override with
...
For `meta.csv` there are two interchangeable copies in the repo — use **either**;
`split.py` normalises the id so it doesn't matter:

- `artifacts/meta.csv` — `occupant_id` as int (`147`)
- `embeddings_arcface/meta.csv` — `occupant_id` as string (`"0000147"`)

**What you do NOT need:**

- `dataset_splits/` — 375 MB of raw face crops. Only steps 01/02 use those, to
  *make* the embeddings. This package never touches an image.
...
```bash
cd lane_a/buildings
python -m venv .venv && .venv\Scripts\activate      # or use lane_a's .venv
pip install -r requirements.txt
```

`requirements.txt` is just `-r ../requirements.txt` → `numpy==2.5.2`,
`pandas==3.0.5`, `matplotlib==3.11.1`. No face model, no torch, no GPU. Install
takes seconds.
...
The 10 committed `out/*.npz` filters were checked against
`lane_a/pipeline/07_evaluate_lsh.py:build_filters` — the function that builds the
routing filters in memory for the 10-building evaluation:
...
ALL 10 BIT-IDENTICAL TO pipeline/07 build_filters
```

This package doesn't reimplement the pipeline — it extracts it. Step 07's
published routing numbers (92.69% correct at 10 buildings, etc.) still describe
...
prototype vectors are the exact privacy regression `docs/project_context.md`
flags against the Bloom filter's one-wayness. Packed bits and integer metadata
only. (Routing — §9 — needs per-occupant reference vectors for voting; it
rebuilds them locally from the corpus every run and never writes them, so this
guarantee on the published `.npz` is untouched.)

**No test framework.** The repo has none — verification lives in
`if __name__ == "__main__":` blocks. This package follows that: every module
self-tests.
...
python -m buildinglib.enroll      # enrollment, no false negatives, error paths
python -m buildinglib.artifact    # npz round-trip, corruption detection
```

**`out/` must not be named `results/` or `artifacts/`.** `lane_a/.gitignore`
lists those without a leading slash, so they match at any depth — a subfolder
with that name would be silently untracked, and the deliverable would never get
committed.
...
`docs/project_context.md`: *"can a visitor be routed to the right building
without any building ever holding or receiving another building's raw biometric
data?"*
