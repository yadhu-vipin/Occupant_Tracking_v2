"""
v6/sim/report.py — Visual evaluation report generator
======================================================

OVERVIEW:
Generates publication-quality high-resolution (150 DPI) Matplotlib graphics visualizing
spatio-temporal tracking metrics, decision threshold trade-offs, and cross-building routing efficiency.

THEORETICAL & VISUALIZATION ARCHITECTURE:
- Plots Precision & Recall curves across decision thresholds $\theta$, marking the optimal operating point $\theta_{opt}$.
- Visualizes local recognition accuracy and F1 score distributions.
- Demonstrates communication efficiency by comparing average building contact counts against full campus broadcast.
- Uses custom color palettes and sans-serif typography suitable for academic paper submission.

KEY CONTRACTS:
- Inputs: `PaperMetricsResult` and `RoutingMetricsResult` dataclass instances.
- Outputs: PNG image files saved to specified reports directory (`reports/*.png`).
"""
...
def _ensure_dir(path: str) -> str:
    """Utility helper ensuring target directory exists before rendering."""
...
def plot_precision_recall(
    ...
    """
    [BREAKPOINT: Precision-Recall Curve Plotter]
    Renders Precision, Recall, and F1 score vs decision threshold theta.

    Annotates the optimal theta intersection point.
    """
    fig, ax = plt.subplots(figsize=(10, 7))
    theta = pm.theta_values

    # [BREAKPOINT 1: Line Series Rendering]
    ...
    # [BREAKPOINT 2: Optimal Theta Marker & Callout]
...
def plot_recognition_performance(
    ...
    """
    [BREAKPOINT: Recognition Performance Summary Bar Chart]
    Renders bar chart comparison for Recognition Accuracy, Precision, Recall, and Max F1.
    """
    ...
    # Render bars and append percentage annotations above each column
...
def plot_buildings_contacted(
    ...
    """
    [BREAKPOINT: Communication Overhead Comparison Chart]
    Renders side-by-side comparison of DSTS targeted routing vs naive full broadcast.
    """
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))

    # [BREAKPOINT 1: Left Subplot — Mean Contact Bar Comparison]
    categories = ["DSTS\nRouting", "Full\nBroadcast"]
    ...
    # [BREAKPOINT 2: Right Subplot — Distribution Histogram]
...
def plot_routing_efficiency(
    ...
    """
    [BREAKPOINT: Routing Accuracy & Summary Table Plotter]
    Renders Rank-1 accuracy pie chart and metric breakdown table.
    """
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))

    # [BREAKPOINT 1: Left Subplot — Rank-1 Accuracy Pie Chart]
    ...
    # [BREAKPOINT 2: Right Subplot — Formatted Summary Table]
    ax2.axis("off")
    table_data = [
...
def generate_evaluation_report(
    ...
    """
    [BREAKPOINT: Complete Report Generation Orchestrator]
    Executes all plotting routines and saves artifact PNGs to disk.

    Args:
        pm: PaperMetricsResult object
        rm: RoutingMetricsResult object
        output_dir: Output folder for generated charts (default "reports")

    Returns:
        Dict mapping chart identifier keys to saved file paths.
    """
